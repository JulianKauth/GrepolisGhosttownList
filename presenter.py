...
class Presenter:
    ...
    def fetch_data(self, server: str, distance: Optional[int]):
        # print(f"Presenter.fetch_data({server})", flush=True)
        self.model.fill_database(server)
        self.display_data(distance)

    def display_data(self, distance: Optional[int]):
        # print("Presenter.display_data()", flush=True)
        self.view.right_side.set_data(self.model.get_ghosts(distance))

    def copy_to_clipboard(self, distance: Optional[int]):
