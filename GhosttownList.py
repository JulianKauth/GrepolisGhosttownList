...
if __name__ == "__main__":
    ...
    # print distance statistics for radius heuristics
    ts = db.cur.execute('SELECT town_id, name, island_x, island_y, points FROM towns').fetchall()
    ts = [Town(*t) for t in ts]
    print("\n".join([str(sqrt((t.x - 500)**2 + (t.y - 500)**2)) for t in ts]))
