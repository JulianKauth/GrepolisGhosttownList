...
class LeftSide(QWidget):
    def __init__(self, parent: QWidget, presenter: Presenter):
        ...
        self.button_fetch_data.clicked.connect(lambda x: self.presenter.fetch_data(self.widget_server.text(), self.distance_as_int()))
        self.widget_server.returnPressed.connect(lambda: self.presenter.fetch_data(self.widget_server.text(), self.distance_as_int()))
        self.widget_distance.returnPressed.connect(lambda: self.presenter.fetch_data(self.widget_server.text(), self.distance_as_int()))
        self.button_copy_to_clipboard.clicked.connect(lambda x: self.presenter.copy_to_clipboard(self.distance_as_int()))
